...
def Add_Run_Details():
    ...
    Add_Run_Details_win.geometry('1920x1200')
    Add_Run_Details_win.title("Add New Run Details")

    # Add widgets and functionality for the new window
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(Add_Run_Details_win,image=img).pack()
    Label(Add_Run_Details_win, text="Online Bus Booking System",fg='dark slate gray',bg='cyan',font=("Arial",16)).pack(padx=10,pady=10)
    Label(Add_Run_Details_win, text="Add Bus Running Details",fg='green',font=("Arial",16)).pack(padx=10,pady=5)

    #%%%%%%%%%%%%%%%%%%%%%
    #setting frame
    fr_run=Frame(Add_Run_Details_win)
    fr_run.pack(padx=5,pady=5)
    #setting labels
    lbl_runbid=Label(fr_run,text='Bus ID')
    lbl_runbid.grid(row=0,column=0)
    lbl_rundate=Label(fr_run,text='Running Date')
    lbl_rundate.grid(row=0,column=2)
    lbl_runseatavailable=Label(fr_run,text='Seat Available')
    lbl_runseatavailable.grid(row=0,column=4)

    #setting textboxes
    ent_runbid=Entry(fr_run)
    ent_runbid.grid(row=0,column=1)
    ent_rundate=Entry(fr_run)
    ent_rundate.grid(row=0,column=3)
    ent_runseatavailable=Entry(fr_run)
    ...
    btn_addrun=Button(fr_run,text='Add Run',bg='light green',command=addrunpopup)
    btn_addrun.grid(row=0,column=6,padx=10)

    #setting delete run button
    btn_deleterun=Button(fr_run,text='Delete Run',bg='light green',command=deleterunpopup)
    btn_deleterun.grid(row=0,column=7)

    def home():
        create_window2(1)
        Add_Run_Details_win.destroy()
    Button(fr_run,text='HOME',borderwidth=5,bg='brown',command=home).grid(padx=5,row=0,column=8)
...
def Add_Route_Details():
    ...
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(Add_Route_Details_win,image=img).pack()
    Label(Add_Route_Details_win, text="Online Bus Booking System",fg='dark slate gray',bg='cyan',pady=5).pack(padx=10,pady=10)
    Label(Add_Route_Details_win, text="Add New Bus Route Details",fg='green').pack(pady=5,padx=10)

    #%%%%%%%%%%%%%%%%%%%%%%%
    #setting frame
    fr_route=Frame(Add_Route_Details_win)
    fr_route.pack(pady=5)
    #setting labels
    lbl_routeid=Label(fr_route,text='Route Id')
    lbl_routeid.grid(row=0,column=0)
    lbl_stnname=Label(fr_route,text='Station Name')
    lbl_stnname.grid(row=0,column=2)
    lbl_stnid=Label(fr_route,text='Station Id')
    ...
    btn_addroute=Button(fr_route,text='Add Route',bg='light green',command=addroute)
    btn_addroute.grid(row=0,column=6)
    #crete delete route button
    btn_deleteroute=Button(fr_route,text='Delete Route',fg='dark slate gray',bg='cyan',command=deleteroute)
    btn_deleteroute.grid(row=0,column=7,padx=5)

    def home():
        create_window2(1)
        Add_Route_Details_win.destroy()
    Button(fr_route,text='HOME',borderwidth=5,bg='brown',command=home).grid(padx=5,row=0,column=8)
...
def Add_Bus_Details():
    ...
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(Add_Bus_Details_win,image=img).pack()
    Label(Add_Bus_Details_win, text="Online Bus Booking System",fg='dark slate gray',bg='cyan').pack(pady=10,padx=10)
    Label(Add_Bus_Details_win, text="Add New Bus Details",fg='green').pack(pady=5,padx=10)

    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #details for bus table
    #setting frame
    fr_bus=Frame(Add_Bus_Details_win)
    fr_bus.pack(pady=5,padx=10)
    #setting labels
    lbl_bid=Label(fr_bus,text="Bus ID")
    lbl_bid.grid(row=0,column=0)
    lbl_btype=Label(fr_bus,text="Bus Type")
    ...
    lbl_bcapacity=Label(fr_bus,text="Capacity")
    lbl_bcapacity.grid(row=0,column=4)
    lbl_bfare=Label(fr_bus,text="Fare Rs")
    lbl_bfare.grid(row=0,column=6)
    lbl_boprid=Label(fr_bus,text="Operator ID")
    lbl_boprid.grid(row=0,column=8)
    lbl_brid=Label(fr_bus,text="Route ID")
    ...
    btn_addbus=Button(fr_bus,text='Add bus',bg='light green',command=addbuspopup)
    btn_addbus.grid(row=1,column=0,columnspan=11)

    #button to edit bus details
    btn_editbus=Button(fr_bus,text='Edit bus',bg='light green',command=editbuspopup)
    btn_editbus.grid(row=1,column=1,columnspan=11)

    def home():
        create_window2(1)
        Add_Bus_Details_win.destroy()
    Button(fr_bus,text='HOME',borderwidth=5,bg='brown',command=home).grid(padx=5,row=1,column=2,columnspan=11)
...
def Add_Operator_Details():
    ...
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(Add_Operator_Details_win,image=img).pack()
    Label(Add_Operator_Details_win, text="Online Bus Booking System",fg='dark slate gray',bg='cyan',pady=5).pack()
    Label(Add_Operator_Details_win, text="Add Bus Operator Details",fg='green').pack()
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #setting frame
    fr_operator=Frame(Add_Operator_Details_win)
    fr_operator.pack()
    #setting labels
    lbl_operatorid=Label(fr_operator,text='Operator id')
    lbl_operatorid.grid(row=0,column=0)
    lbl_name=Label(fr_operator,text='Name')
    lbl_name.grid(row=0,column=2)
    lbl_address=Label(fr_operator,text='Address')
    lbl_address.grid(row=0,column=4)
    lbl_phone=Label(fr_operator,text='Phone')
    lbl_phone.grid(row=0,column=6)
    lbl_email=Label(fr_operator,text='Email')
    ...
    btn_opradd=Button(fr_operator,text='Add',bg='light green',command=opraddpopup)
    btn_opradd.grid(row=0,column=10,padx=5)

    #button to edit operator details
    btn_opredit=Button(fr_operator,text='Edit',bg='light green',command=opreditpopup)
    btn_opredit.grid(row=0,column=11)

    def home():
        create_window2(1)
        Add_Operator_Details_win.destroy()
    Button(fr_operator,text='HOME',borderwidth=5,bg='brown',command=home).grid(padx=5,row=0,column=12)
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
def Add_Database_Details():
    ...
    Add_Database_Details_win.geometry('1920x1200')
 

    # Add widgets and functionality for the new window
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(Add_Database_Details_win,image=img).pack()
    Label(Add_Database_Details_win, text="Online Bus Booking System",fg='dark slate gray',bg='cyan',pady=5).pack()
    Label(Add_Database_Details_win, text="Add New Details to DataBase",fg='green').pack()
    #creating a frame for 4 buttons
    fr1=Frame(Add_Database_Details_win)
    fr1.pack()
    
    #adding buttons
    btn_newoperator=Button(fr1,text='New Operator',bg='light green',command=Add_Operator_Details)
    btn_newoperator.grid(row=0,column=0)
    btn_newbus=Button(fr1,text='New Bus',bg='orange',command=Add_Bus_Details)
    btn_newbus.grid(row=0,column=1)
    btn_newroute=Button(fr1,text='New Route',bg='sky blue',command=Add_Route_Details)
    btn_newroute.grid(row=0,column=2)
    btn_newrun=Button(fr1,text='New Run',bg='purple',command=Add_Run_Details)
    btn_newrun.grid(row=0,column=3)

    def home():
        create_window2(1)
        Add_Database_Details_win.destroy()
    Button(fr1,text='HOME',borderwidth=5,bg='brown',command=home).grid(padx=5,row=0,column=4)
...
def Check_Booked_Seat():
    ...
    check_Booked_win.geometry('1920x1200')

    # Add widgets and functionality for the new window
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(check_Booked_win,image=img).pack()
    Label(check_Booked_win, text="Online Bus Booking System",fg='dark slate gray',bg='cyan',pady=5).pack()
    Label(check_Booked_win, text="Check Your Booking",bg='light green').pack()

    #create frame
    fr_detail=Frame(check_Booked_win)
    fr_detail.pack()
    Label(fr_detail,text='Enter Your Mobile No:').grid(row=0,column=0)
    ...
    btn1=Button(fr_detail,text='Check Booking',command=check_entry)
    btn1.grid(row=0,column=2)
    def home():
        create_window2(1)
        check_Booked_win.destroy()
    Button(fr_detail,text='HOME',borderwidth=5,bg='brown',command=home).grid(padx=5,row=0,column=3)
...
def ticket_window(ent_mobile):
    ...
    ticket_win.geometry('1920x1200')

    # Add widgets and functionality for the new window
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(ticket_win,image=img).pack()
    Label(ticket_win, text="Online Bus Booking System",fg='dark slate gray',bg='cyan').pack()
    Label(ticket_win, text="Bus ticket").pack()
...
def Seat_Booking():
    ...
    Seat_Booking_win.geometry('1920x1200')

    # Add widgets and functionality for the new window
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(Seat_Booking_win,image=img).pack()
    Label(Seat_Booking_win, text="Online Bus Booking System",bg='sky blue',fg='red',font=('Arial',16)).pack(pady=10)
    Label(Seat_Booking_win, text="Enter Journey Details",bg='light green',font=('Arial',10)).pack()
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #making a show frame
    fr_show=Frame(Seat_Booking_win)
    fr_show.pack(pady=10)
    #putting widgets
    lbl_to=Label(fr_show,text='To')
    lbl_to.grid(row=0,column=0)
    lbl_from=Label(fr_show,text='From')
    lbl_from.grid(row=0,column=2)
    lbl_jdate=Label(fr_show,text='Journey Date')
    ...
    def proceedtobook(selected_bid):
        ...
        def popup():
            ...
            conn.close()
        #button to book seat
        btn_bookseat=Button(fr_passenger,text='Book Seat',bg='light green',command=popup)
    ...
    def showbus():
        ...
        cur.execute('''select distinct bus.B_id,operator.Opr_name,bus.B_type,run.Run_seatavailable,bus.B_capacity,bus.B_fare
                    ...
                    where (route.Stn_name=? or route.Stn_name=?) and run.Run_date=?
                    ''',(ent_to.get(),ent_from.get(),ent_jdate.get()))
        final=cur.fetchall()
    ...
    btn_showbus=Button(fr_show,text='Show Bus',bg='green',command=showbus)
    btn_showbus.grid(row=0,column=6)

    def home():
        create_window2(1)
        Seat_Booking_win.destroy()
    Button(fr_show,text='HOME',bg='brown',borderwidth=5,command=home).grid(padx=5,row=0,column=7)
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
def create_window2(event):
    ...
    global img
    img=ImageTk.PhotoImage(Image.open('bus.jpg').resize((300, 200)))
    Label(window2,image=img).pack()
    Label(window2, text="Online Bus Booking System",fg='dark slate gray',bg='cyan').pack()
    ...
    btn1=Button(fr1,text='Seat Booking',bg='RosyBrown3', fg='white',command=Seat_Booking) 
    btn1.grid(row=0,column=0)
    
    btn2=Button(fr1,text='Check Booked Seat',bg='RosyBrown3', fg='white',command=Check_Booked_Seat)
    btn2.grid(row=0,column=1)
    
    btn3=Button(fr1,text='Add Database Details',bg='RosyBrown3', fg='white',command=Add_Database_Details)
    btn3.grid(row=0,column=2)

    Label(fr1,text='For Admin Only',fg='red').grid(row=1,column=2)
...
label1 = Label(root, text="Online Bus Booking System",fg='dark slate gray',bg='cyan',pady=10)
label1.pack()
Label(root,text="Name:Vidit Tamrakar",fg='dark slate gray').pack()
Label(root,text="Er : 221B440",fg='dark slate gray').pack()
Label(root,text="Mobile: 9354607249",fg='dark slate gray').pack()
Label(root,text="Submitted to: Dr. Mahesh Kumar",fg='white',bg='cyan4').pack()
Label(root,text="Project Based Learning",fg='dark slate gray').pack()
